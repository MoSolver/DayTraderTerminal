...
function IVSurface3D({ callsData, putsData }) {
  ...
  useEffect(() => {
    ...
    sceneRef.current.buildSurface = buildSurface;
    sceneRef.current.buildMeshWithData = (mode, calls, puts) => {
      const { THREE, scene, inited } = sceneRef.current;
      if (!inited) return;
      ...
      const rows = mode === 'calls' ? calls : puts;
      const surface = sceneRef.current.buildSurface(rows);
      if (!surface) return;
  ...
  // Rebuild mesh when mode or data changes
  useEffect(() => {
    if (sceneRef.current.buildMeshWithData) {
      sceneRef.current.buildMeshWithData(surfaceMode, callsData, putsData);
    }
    ...
    <div className="panel">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          <button className="btn" onClick={() => sceneRef.current._resetCamera && sceneRef.current._resetCamera()}>
            ↺ RESET
          </button>
        </div>
      </div>
